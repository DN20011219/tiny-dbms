...
root用户可以访问该数据库，并执行查询sql，但不可手动修改其中的数据，root用户可以通过执行创建数据库的语句对其进行修改。
普通用户无法访问该数据库，也无法创建新的数据库，只能在连接时选择已存在的数据库。
...
## 3.3、客户端-服务器架构
由于本系统是一个简单的模拟数据库，相较于复杂的网络模块（复杂原因包括：网络环境中的可靠问题、协议选择、debug难度、代码实现），我选择了使用消息队列实现客户端和服务器之间的可靠通讯。下图为本系统的cs架构图：
